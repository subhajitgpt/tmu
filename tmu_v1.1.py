...
def train_and_tune(df: pd.DataFrame):
    ...
    proba_train = best.predict_proba(Xtr)[:, 1]
...
def importance_tables(model, df: pd.DataFrame):
    ...
    imp = pd.DataFrame({"feature": FEATURES, "coef": coef, "abs_importance": np.abs(coef)}) \
...
def score_all(model, df: pd.DataFrame) -> pd.DataFrame:
    ...
    Xs = scaler.transform(X); contrib = Xs * clf.coef_[0]
...
<head>
  ...
  <title>Risk Insights Studio — Flask (ROC + Top-k)</title>
...
if __name__ == "__main__":
    ...
    # Important: use_reloader=False so it runs once (no double start)
