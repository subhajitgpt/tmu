...
def generate_dummy(n: int = 50_000) -> pd.DataFrame:
...
def train_and_tune(df: pd.DataFrame):
    ...
    # Robust grid with strong regularization and bagging
    param_grid = [
        {"depth": 3, "learning_rate": 0.03, "l2_leaf_reg": 10.0, "min_data_in_leaf": 100},
        {"depth": 3, "learning_rate": 0.05, "l2_leaf_reg": 15.0, "min_data_in_leaf": 80},
        {"depth": 4, "learning_rate": 0.03, "l2_leaf_reg": 12.0, "min_data_in_leaf": 80},
        {"depth": 4, "learning_rate": 0.05, "l2_leaf_reg": 15.0, "min_data_in_leaf": 60},
    ]
    base = dict(
    ...
    train_class_counts = pd.Series(ytr).value_counts().sort_index()
    test_class_counts = pd.Series(yte).value_counts().sort_index()
    
    class_dist_data = {
        "Split": ["Train", "Test"],
        "Total": [len(ytr), len(yte)],
        "Class 0 (Safe)": [train_class_counts.get(0, 0), test_class_counts.get(0, 0)],
        "Class 1 (Risky)": [train_class_counts.get(1, 0), test_class_counts.get(1, 0)],
        "Class 1 %": [f"{ytr.mean()*100:.2f}%", f"{yte.mean()*100:.2f}%"]
    }
    ...
    return final_model, {
        ...
        "topk_train_html": topk_train_df.to_html(classes="table table-sm table-striped table-hover align-middle",
                                                 index=False, border=0, justify="left"),
        "class_dist_html": class_dist_df.to_html(classes="table table-sm table-striped table-hover align-middle",
                                                  index=False, border=0, justify="left"),
...
def bucket(p: float) -> str:
    if p >= 0.90 and p <= 1.0: return "Very High"
    if p >= 0.80 and p < 0.90: return "High"
    if p >= 0.70 and p < 0.80: return "Moderate"
    if p >= 0.50 and p < 0.70: return "Low"
    return "No Risk"
...
def score_all(model: CatBoostClassifier, df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["probability"] = (out["prob"] * 100).round(2).astype(str) + "%"
    
    # Pure score-based bucketing with explicit ranges
    def score_bucket(p: float) -> str:
        if p >= 0.90 and p <= 1.0: return "Very High"
        if p >= 0.80 and p < 0.90: return "High"
        if p >= 0.70 and p < 0.80: return "Moderate"
        if p >= 0.50 and p < 0.70: return "Low"
    ...
    out["bucket"] = [score_bucket(p) for p in proba]
...
# ---------- Removed AI panel functionality ----------
# ---------- Secrets & AI helpers ----------
# Removed OpenAI integration functions
    ...
    <div class="col-lg-8">
      ...
      <div class="tab-content pt-3">
        ...
        <div class="tab-pane fade" id="trainperf"> 
          <div class="card card-soft mb-3">
            <div class="card-body">
              <h5 class="mb-2"><i class="bi bi-pie-chart"></i> Class Distribution</h5>
              {% if perf and perf.class_dist_html %}
                <div class="table-responsive">{{ perf.class_dist_html|safe }}</div>
                <div class="small text-muted mt-2">
                  <b>Model Configuration:</b> {{ perf.total_features }} features | Best AUC: {{ '%.4f'|format(perf.val_auc) }} | Test Accuracy: {{ '%.2f'|format(perf.test_acc*100) }}%
                </div>
              {% else %}
                <p class="text-muted">Train to view class distribution.</p>
              {% endif %}
            </div>
          </div>
          <div class="card card-soft mb-3">
